...
public class DoctorIntroductionProcessor2 implements PageProcessor {
...
//	private DBObject dbObject;
	...
	public DoctorIntroductionProcessor2(String intro_url) {
...
//		key.put("desc",new BasicDBObject("$exists",false));
		...
		for(int i = 0; i != numThread; i++){
			...
			executorService.execute(new SpiderRunnable2(subList));
		}
...
class SpiderRunnable2 implements Runnable{
	
	final List<DBObject> list;
	
	public SpiderRunnable2(List<DBObject> list){
		this.list = list;
	}
...
//		int sum = 0;
		for (DBObject record : list) {
			...
			if(null != url && !"".equals(url)){
				Spider.create(new DoctorIntroductionProcessor2(url))
