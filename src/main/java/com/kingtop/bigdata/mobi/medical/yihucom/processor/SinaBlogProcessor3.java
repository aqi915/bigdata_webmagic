...
/**
 * 先到mongodb查询出上目标网站，再去查找要的东西
 * 
 * @author hyq
 *
 */
public class SinaBlogProcessor3 implements PageProcessor {
	...
	public void process(Page page) {
		...
		MongoDaoImpl.getInstance().insert("blog_article3", object);
	}
	...
	public static void main(String[] args) {
		List<DBObject> records = MongoDaoImpl.getInstance().findAll("blog_article");

		for (DBObject record : records) {
			if ((String) record.get("blog_url") != null&(String) record.get("blog_url") != "") {
				Spider.create(new SinaBlogProcessor3()).addUrl((String) record.get("blog_url")).thread(5).run();
