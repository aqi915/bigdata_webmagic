...
public class answerSpider implements PageProcessor {
	...
	private void processProvince(Page page) {
		...
		List<String> districts = page.getHtml().xpath("//body/table[@class='t12']/tbody/tr/td")
		...
		for (String district : districts) {
			...
			Request request = new Request(link).setPriority(1)
					.putExtra("province", page.getRequest().getExtra("province")).putExtra("district", title);
			page.addTargetRequest(request);
		}
	...
	private void processDistrict(Page page) {
		...
		int i = counties.size();
		for (int j = 0; j < i; j++) {
			System.out.println(province + " " + district + " " + counties.get(j) + " 邮编：" + youbians.get(j) + " 区号："
					+ quhaos.get(j));
		}

		/*
		 * int i=counties.size(); for(int j=0;j<i;j++){
